...
from src.utils.app_utils import create_directory
...
class ChatBot:
    ...
    Attributes:
        configs (dict): A configuration dictionary that stores specific settings such as the `thread_id`.
    ...
    def respond(chatbot: List, message: str) -> Tuple:
        ...
        """
        # The configs is the **second positional argument** to stream() or invoke()!
        events = graph.stream(
